...
?>
<div class="container">
<form method='post' action='downloadNotCheckedOut.php'>
<div class="report_top_group">
<h1>Equipment Not Checked Out</h1>
<div id="downloadBttn" >
...
<?php
$sql = mysqli_query($connection,"Select distinct c.equipID, eq.type, c.checkinDate, c.checkoutDate, c.quantityOut From checkoutLog as c, equipment as eq Where eq.equipID = c.equipID and c.checkinDate is not Null and c.checkoutDate is not Null ORDER By eq.type");
$user_arr = array(array('Equipment ID', 'Equipment Type', 'Quantity Available'));

while($record = mysqli_fetch_assoc($sql)) {
	$user_arr[] = array($record['equipID'], $record['type'], $record['quantityOut']);
	echo "<tr><td>" . $record['equipID'] . "</td><td>" . $record['type'] . "</td><td>" . $record['quantityOut'] . "</td></tr>";	
}
?>
</table>
