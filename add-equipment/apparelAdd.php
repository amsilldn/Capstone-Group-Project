...
if ($id=='' OR $size=='' OR $quant=='') {
	$check = '<b>Fill out all fields.</b>'; //<p>ID:' . $id . '<p>Name:' . $name . '<p>Size:' . $size . '<p>Quantity:' . $quan . "."
}

else {

		//ADDING QUANTITY
		$sql="UPDATE equipment SET quantityOwned = quantityOwned +" . $quant . " WHERE (equipID = '" . $id . "' AND size = '" . $size ."')";
...
<body>
    ...
    <div class="bContainer">
        <div class="aAddForm">
            <!-- populate h2 with apparel item from database -->
            <h2>Apparel</h2>
            <h4>Fill out all fields.</h4>
            <div class="aAddButtons">
                <form action="#" method="POST">
                    <div class="one">
                    <label for="id">ID #: </label><input type=text name="id" id="id" size="54" />
                    <button class="btn" type="button" id="button" data-toggle="modal" data-target="#livestream_scanner">
								<i class="fas fa-barcode"></i>
							</button>
                </div>
                <div class="two"
                    <label for="size">Size: </label><input type=text name="size" id="size" size="54" />
                </div>
                <div class="three">
                    <label for="quant">Quantity: </label><input type=text name="quant" id="quant" size="50" />
                </div>
                    <div class="sButton">
                    <button class="bttn" type="submit" name="aAdd" id="aAdd">Add Apparel</button>
                </div>
                </form>
            </div>
        </div>
