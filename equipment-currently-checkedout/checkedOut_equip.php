...
?>
<div class="container">
<form method='post' action='downloadCheckedOutEq.php'>
...
$sql = mysqli_query($connection,"Select c.equipID, eq.type, c.quantityOut, c.checkoutDate, c.event, c.hasEar From checkoutLog as c, equipment as eq Where checkinDate is null and eq.equipID = c.equipID");
$user_arr = array(array('Equipment ID', 'Equipment Type', 'Employee', 'Qty Checked Out', 'Checkout Date', 'Event', 'Earpiece'));
while($record = mysqli_fetch_assoc($sql)) {
	$arr = array(1 => 'Yes', 0 => 'No');
	$user_arr[] = array($record['equipID'], $record['type'], $record['employee'], $record['quantityOut'], $record['checkoutDate'], $record['event'], $arr[$record['hasEar']]);
	echo "<tr><td>" . $record['equipID'] . "</td><td>" . $record['type'] . "</td><td>" . $record['quantityOut'] . "</td><td>" . $record['checkoutDate'] . "</td><td>" . $record['event'] . "</td><td>" . $arr[$record['hasEar']] . "</td></tr>";	
}
?>
</table>
