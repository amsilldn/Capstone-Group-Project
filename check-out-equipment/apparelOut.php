<?php
	...
	$apparel = $_SESSION['checkout_apparel']; //take the session variable to know what they selected on checkout.php
	
	if (isset($_POST['rCheckOut'])) {
		...
		$get_quant = mysqli_query($connection, "SELECT quantityOwned FROM equipment WHERE type = '" . $apparel . "' AND size = '" . $size . "'");
		$quantity_owned = mysqli_fetch_assoc($get_quant)['quantityOwned'];
		
		//then see how many of that item are current checked out (and not checked in)
		$get_checked_out = mysqli_query($connection, "SELECT SUM(cl.quantityOut) as 'Out' FROM checkoutLog as cl, equipment as e WHERE e.equipID = cl.equipID AND e.type = '" . $apparel . "' AND e.size = '" . $size . "' AND checkinDate IS NULL");
		$quantity_out = mysqli_fetch_assoc($get_checked_out)['Out'];
		
		//If quantity out is returned NULL, then assign it to 0
		if ($quantity_out==""){
			$quantity_out=0;
		}
		//error handling in case the user enters in anything invalid
		try {
			$owned = intval($quantity_owned);
			//find the actual quantity available
			$quantity_available = ($owned - $quantity_out);
			...
			else if ($quantity_available < $quant){
			...
			//otherwise, go ahead and insert the record into the checkout log, with the current date stamp
				...
				$insert = "INSERT INTO checkoutLog(equipID, empID, quantityOut, checkoutDate, event) VALUES('" . $id . "', '" . $emp . "','" . $quant . "',NOW(), '" . $event . "')";
				if (mysqli_query($connection, $insert)) {
					$output = "It worked!";
					//header("location:checkout.php");//redirect to comment page
				}
				else{
...
<nav id="sidenav">
    ...
    <div class="bContainer">
        <div class="aOutForm">
            <div class="aOutButtons">
                <form action="apparelOut.php" method="POST">
				<h2> <?php echo $apparel; ?> </h2>
...
<?php
	$get_size = mysqli_query($connection, "SELECT size FROM equipment WHERE type = '" . $apparel . "'");
...
?>
                    ...
                    <div class="three">
						<label for="emp">Employee: </label><input type=text name="emp" id="emp" size="50" />
							<button class="btn" type="button" id="button2" data-toggle="modal" data-target="#livestream_scanner">
								<i class="fas fa-barcode"></i>
							</button>
</select>					
					<!--<input type=text name="emp" id="emp" size="50" />-->
					</div>
					<div class="four">
                    <label for="quant">Quantity: </label><input type=text name="quant" id="quant" size="50" />
                    </div>
                    <div class="five">
					<label for="event">Event: </label><input type=text name="event" id="event" size="50" />
					</div>
					<div class="sButton">
                    <button class="bttn" type="submit" name="rCheckOut" id="rCheckOut">Check Out</button>
                    </div>
