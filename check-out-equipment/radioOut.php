<?php
	...
	if (isset($_POST['rCheckOut'])) {
		...
		$ear = mysqli_real_escape_string($connection, $_POST['ear']);
		...
		//make sure that radio isn't already checked out
		$check = mysqli_query($connection, "SELECT checkoutDate, checkinDate FROM checkoutLog WHERE equipID='" . $id . "' ORDER BY checkoutDate DESC");
		$checkoutDate = mysqli_fetch_assoc($check)['checkoutDate'];
		$checkinDate = mysqli_fetch_assoc($check)['checkinDate'];
		//because it was sorted by checkoutDate descending, it means it will pull the most recent record.
		//if it has a checkout date but not a checkin date, that means it is currently checked out and can't be checked out again
		if (!empty($checkoutDate) and empty($checkinDate)){
				#$output = $checkinDate;
				$output = "Item already checked out.";
		}
		...
		//or if they try to enter an equipment ID that doesn't exist
			...
			$insert = "INSERT INTO checkoutLog(equipID, empID, hasEar, checkoutDate, event) VALUES('" . $id . "', '" . $emp . "','" . $ear . "',NOW(), '" . $event. "')";
			if (mysqli_query($connection, $insert)) {
				$output = "Radio Checked Out";
...
<body>
    ...
    <div class="container">
        ...
        <div class="rOutForm">
            ...
            <div class="rOutButtons">
                <form action="radioOut.php" method="POST">
                    <div class="one">
                ...
                <div class="two">
                    <label for="ear">Earpiece: </label><input type=radio name="ear" id="yes"/>Yes  <input type=radio name="ear" id="no"/>No
                </div>
                <div class="three">
                ...
                <div class="four">
                    <label for="emp">Event: </label><input type=text name="event" id="event" size="54" />
                </div>
