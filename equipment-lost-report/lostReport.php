...
?>
<div class="container">
<form method='post' action='downloadLostReport.php'>
...
<?php
$sql = mysqli_query($connection,"Select distinct l.equipID, eq.type, concat(e.firstName, ' ', e.lastName) as name, l.quantityLost, l.reportDate, l.equipID From lostLog as l, employee as e, equipment as eq Where foundDate is null and e.empID=l.empID and eq.equipID = l.equipID Order by name");
$user_arr = array(array('Equipment ID', 'Equipment Type', 'Reported By', 'Quantity Lost', 'Report Date'));
while($record = mysqli_fetch_assoc($sql)) {
	$user_arr[] = array($record['equipID'], $record['type'], $record['name'], $record['quantityLost'], $record['reportDate']);
	echo "<tr><td>" . $record['equipID'] . "</td><td>" . $record['type'] . "</td><td>" . $record['name'] . "</td><td>" . $record['quantityLost'] . "</td><td>" . $record['reportDate'] . "</td></tr>";	
}
?>
</table>
